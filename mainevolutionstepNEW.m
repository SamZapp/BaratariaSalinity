...
MannHR=0*A+CbR;
%MannHR(VEGfp==1)=CbR*5;


MannSmud=0*A+CbS_MUD;
%MannSmud(VEG==1)=CbSveg_MUD;
%MannSmud(VEGfp==1)=CbS_MUD*5;

MannSsand=0*A+CbS_SAND;
%MannSsand(VEGfp==1)=CbS_SAND*5;%CbSveg_SAND;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if usepreviousFLOW==1 & length(hpRIVo)>1;hpRIV=hpRIVo;Uref=Uo;
else
hpRIV=max(0,Zlev+initialhforriverflow);%+5; %at the beginning, just a small water level from the river to avoid extra slopes
end
else;hpRIV=0;end
...
if computeRiver==1;
    Uo=A*0+Uref;%first attempt of velocity
    %Uo=FcrUR*sqrt(9.81*h)*0.5;%first attempt of velocity
    DD1=A*0;DDN=A*0;
    [UR,URx,URy,hriver,qR1,qRm1,qRN,qRmN,UR1,URm1,URN,URmN]=riverFLOWiter(A,MannHR,h,ho,dx,fTide,Qmouth,Uo,directionQ,imposenocrossflux,FcrUR,DD1,DDN,DDUlimit);%hriver=max(hriver, max(0,-z));
...
%             hpRIV=(hpRIV+hriver)/2;
%             [h,ho,fTide,DHE,wl,wlo]=getwaterdepth(Trange,msl,z,kro,hpRIV,tempdeltaMSL);  
%             if nonlinearfriction==1;Uo=max(0.1,(Uo*0.9+UR*0.1));end
%             end
...
%             end
%             
%             [UR,URx,URy,hriver,qR1,qRm1,qRN,qRmN,UR1,URm1,URN,URmN]=riverFLOWiter(A,MannHR,h,ho,dx,fTide,Qmouth,Uo,directionQ,imposenocrossflux,FcrUR,DD1,DDN,DDUlimit);%hriver=max(hriver, max(0,-z));PLT.hriver=hriver;
%             end            
%             %EX=max((h(:)>1).*abs(hriver(:)-hpRIV(:)))
            ...
            if riverwaterlevel==1  
            %hpRIV=(hpRIV+hriver)/2;
            %hpRIV=(hpRIV*0.8+hriver*0.2);
            %fac=max(0.1,min(h/5,0.5));%            fac=max(0.1,min(h,1));
            %fac=max(0.05,min(h/5,0.5));%            fac=max(0.1,min(h,1));
            %fac=max(0.1,min(h/5,0.5));%            fac=max(0.1,min(h,1));
            fac=max(0.01,min(h/10,0.5));%            fac=max(0.1,min(h,1));
            %fac=max(0.1,min(h/5,0.5));%            fac=max(0.1,min(h,1));
            ...
            %if nonlinearfriction==1;Uo=max(0.5,(Uo*0.8+UR*0.2));end
            if nonlinearfriction==1;Uo=max(0.5,(Uo*0.5+UR*0.5));end
            end
            %if  correctmomentumriver==1 & i==nitero; %only to it the last iteration
            %DD1=facmomentum*(UR1-URm1)/dx.*(h>0); DDN=facmomentum*(URN-URmN)/dx.*(h>0);
            %end
            ...
            [UR,URx,URy,hriver,qR1,qRm1,qRN,qRmN,UR1,URm1,URN,URmN]=riverFLOWiter(A,MannHR,h,ho,dx,fTide,Qmouth,Uo,directionQ,imposenocrossflux,FcrUR,DD1,DDN,DDUlimit);%hriver=max(hriver, max(0,-z));PLT.hriver=hriver;
            %EX=max((h(:)>2 & A(:)~=0).*abs(hriver(:)-hpRIV(:)));            %EX= prctile((h(:)>2 & A(:)~=0).*abs(hriver(:)-hpRIV(:)),99.99);
            EX=max((h(:)>2).*abs(hriver(:)-hpRIV(:)));            %EX= prctile((h(:)>2 & A(:)~=0).*abs(hriver(:)-hpRIV(:)),99.99);
            EX=max(EX,10*max((UR(:)>0.5).*(h(:)>2).*abs(Uo(:)-UR(:))));            %EX= prctile((h(:)>2 & A(:)~=0).*abs(hriver(:)-hpRIV(:)),99.99);
            %EX=max(EX,2*abs(hriver(A>=10 & A<=20)-hpRIV(A>=10 & A<=20)));
            cont=cont+1;  
            end         
            [cont EX]%figure;imagesc(hriver-hpRIV);pause
...
DD1=facmomentum*(U1-Um1)/dx.*(h>0); DDN=facmomentum*(UN-UmN)/dx.*(h>0);
DD1(A==2)=0;DDN(A==2)=0;
...
[Uwave_seaI,Tp_seaI,Hsea,Fetch,PWsea,QsWslope_sea]=SeaWaves_multipleheightHHH(A,WINDdir,hwSea_lim,Trange,WINDspeed,VEG,64,Nhseawave,dx,z,msl,tempdeltaMSL,ws1,fTide,extraHseaimposed,addextrafetch,extrafetch); %72,dx
...
%Advection-Diffusion Sediment transport
    if computesand==1;
        WS=A*0+ws1;
        flagSANDMUD=0;
           [EmD1,SSM,FLX1,XXTide,XXRiver]=sedtran(flagSANDMUD,h,A,SPCLcell,0,DiffSsand,Zlev,h,ho,E1,WS,dx,dt,rbulk1,co1,SeaSSCbelowLIMIT,ZlevcoLIMIT,Ux,Uy,fTide,Ttide,qR1,qRm1,qRN,qRmN,periodic,computeRiver,computetidalcurrent,residualcurrents,kro,co1mouth,FLX1,tracksedimentfluxes,XX); 
    ...
    if computemud==1;
        ...
        flagSANDMUD=1;  
        ADoMUD=A*0+DoMUD;
        ADoMUD(VEG==1)=DoMUD+DoMUDsubgridVEG;
...
Qs1=E1o./(ws1*3600*24).*(U*alphaSAND+UR*alphaSANDriver).*max(hlimCdwn,h); %kg/m/s
...
Qs2=E2o./(ws2*3600*24).*(U+UR*0.1).*max(hlimCdwn,h); %kg/m/s 
...
%g=max(abs(znew(A==10)-zoriginal(A==10)));g=g>1;g
%maxup=max(maxup,g*1000);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
