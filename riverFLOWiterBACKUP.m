...
% if abs(k)==1
%     DD=1./(Icsi+sign(DD1).*min(Icsi*DDUlimit,(h.*abs(DD1))/9.81))   .*h.^2/(dx^2);
% elseif abs(k)==N
%     DD=1./(Icsi+sign(DDN).*min(Icsi*DDUlimit,(h.*abs(DDN))/9.81))   .*h.^2/(dx^2);
% end
% DD=(DD(p(a))+DD(q(a)))/2;
...
%DDUm=(DDU(p(a))+DDU(q(a)))/2;DDUm=sign(DDUm).*min(abs(DDU(p(a))),abs(DDU(q(a))));
Icsim=(Icsi(p(a))+Icsi(q(a)))/2;
%Icsim=max(Icsi(p(a)),Icsi(q(a)));
%Icsim=1./max(csi(p(a)),csi(q(a)));
hm=(h(p(a))+h(q(a)))/2;
%hm=max(h(p(a)),h(q(a)));
%DD=1./(Icsim+sign(DDUm).*min(Icsim*DDUlimit,abs(DDUm)))   .*hm.^2 /(dx^2);
%DD=1./(Icsim+max(-Icsim*DDUlimit,DDUm))   .*hm.^2 /(dx^2);
%DD=1./(Icsim+DDUm)   .*hm.^2 /(dx^2);
DD=1./(Icsim+max(-0.9*Icsim,DDUm))   .*hm.^2 /(dx^2);
%hm=(h(p(a)).^2+h(q(a)).^2)/2;
%DD=1./(Icsim+sign(DDUm).*min(Icsim*DDUlimit,abs(DDUm)))   .*hm /(dx^2);
...
% if abs(k)==1
%     DD=1./(Icsi+sign(DD1).*min(Icsi*DDUlimit,(h.*abs(DD1))/9.81))   .*h.^2/(dx^2)  *dx;
% elseif abs(k)==N
%     DD=1./(Icsi+sign(DDN).*min(Icsi*DDUlimit,(h.*abs(DDN))/9.81))   .*h.^2/(dx^2)  *dx;
% end
% DD=(DD(p(a))+DD(q(a)))/2;
...
hm=(h(p(a))+h(q(a)))/2;
%hm=max(h(p(a)),h(q(a)));
%DD=1./(Icsim+sign(DDUm).*min(Icsim*DDUlimit,abs(DDUm)))   .*hm.^2 /(dx^2) *dx;
%DD=1./(Icsim+max(-Icsim*DDUlimit,DDUm))   .*hm.^2 /(dx^2) *dx;%DDUlimit
DD=1./(Icsim+max(-0.9*Icsim,DDUm))   .*hm.^2 /(dx^2) *dx;
%DD=1./(Icsim+DDUm)   .*hm.^2 /(dx^2) *dx;
%hm=(h(p(a)).^2+h(q(a)).^2)/2;
%DD=1./(Icsim+sign(DDUm).*min(Icsim*DDUlimit,abs(DDUm)))   .*hm /(dx^2) *dx;
