...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Sea level%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
max_add_RSLR=1/1000/365;
trend=(0:1/(length(A(:,1)))*max_add_RSLR:max_add_RSLR)';
svRSLR=repmat(trend(1:end-1),[1 length(A(1,:))]);
RSLR=ones(size(A)).*RSLR+svRSLR;
...
Y1(A>=10 & A<=19)=Y1(A>=10 & A<=19)+RSLR(A>=10 & A<=19)*dt; %sediment on the bed of the river mouth!!
z=zs+(Y1+Y2+Y3);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             %EX=max((h(:)>1).*abs(hriver(:)-hpRIV(:)))
            ...
            minUfricACT=max(minUfric,0.5*(h<2));
...
if computetidalcurrent==1;
    ...
    pondleakage=0.2;
    DHE(S==1)=max(0, Trange(S==1)/2-(z(S==1)-msl(S==1))-max(0,DIF(S==1)-pondleakage));
...
[Uwave_seaI,Tp_seaI,Hsea,Fetch,PWsea,QsWslope_sea]=SeaWaves_multipleheightHHHdx100(A,WINDdir,hwSea_lim,Trange,WINDspeed,VEG,64,Nhseawave,dx,z,msl,tempdeltaMSL,ws1,fTide,extraHseaimposed,addextrafetch,extrafetch); %72,dx
MASK=A*0;
[Hsea]=getextraHseafordx100(Hsea,h,MASK,dx,N,M);
%Hsea(HS>0)=HS;
...
else;Uwave_sea=0*A;Hsea=0*A;Fetch=0*A;PWsea=A*0;QsWslope_sea=0*A;Uwave_seaI=NaN;Tp_seaI=NaN;end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%size(Uwave_seaI)
