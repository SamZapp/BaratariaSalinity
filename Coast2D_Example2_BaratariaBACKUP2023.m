...
%Sea level rise
P.RSLR=(3+4)/1000/365;  %from mm/yr to m/day (the time unit is the day!)
...
P.AccreteOrganic=1;
P.Korg=8/1000/365;%8/1000/365;%P.Korg=org/365;%5/1000/365;
...
Qmouth_a=5*0.1*0.5;%1*0; %river discharge per unit of cell [m2/s]
...
Qmouth_b=10*0.1*1*0.5; %river discharge per unit of cell [m2/s]
...
Qmouth_c=10*0.1*1*0.5;%5*0; %river discharge per unit of cell [m2/s]
...
Qmouth_d=5*0.2*1*0.5  *0.5; %river discharge per unit of cell [m2/s]
co2mouth_d=0/1000.*P.fMFriver;%500/1000; %SSC of mud at the river [g/l]
directionQ_d=2; %1:up -1:down   2:left  -2:right


P.Qmouth=[Qmouth_a Qmouth_b Qmouth_c Qmouth_d];
P.hmouth=[hmouth_a hmouth_b hmouth_c hmouth_d];
P.co2mouth=[co2mouth_a  co2mouth_b co2mouth_c co2mouth_d];
P.co1mouth=[0 0 0 0];
P.directionQ=[directionQ_a directionQ_b directionQ_c directionQ_d];
...
v=VideoWriter('BaratariaVIDEO_newwave_TEST23','Motion JPEG AVI');
...
for t=1:350;%500;%tmax;  %iteration over the tmax time stpes
...
while dti<dto;
    firstattemp=1;maxdeltaz=limitdeltaz+1;maxup=limitmaxup+1;
        ...
        %[IOtemp,fIOtemp,maxdeltaz,maxup,PLT]=mainevolutionstep_backUP(A,AW,SPCLcell,P,x,dx,dt,IO,fIO);
        %[IOtemp,fIOtemp,maxdeltaz,maxup,PLT]=mainevolutionstep_backUP(A,AW,SPCLcell,P,x,dx,dt,IO,fIO);
...
awVAR(SALTCo<=0.2)=P.aw;%*0.5;
awVAR(SALTCo>0.2 & SALTCo<5)=P.aw*5;
awVAR(SALTCo>=5 & SALTCo<10)=P.aw*2;
P.awVAR=awVAR;

pondVAR(SALTCo>=0.5)=1;
pondVAR(SALTCo<0.5)=0;
P.pondVAR=pondVAR;

KorgVAR(SALTCo<0.2)=P.Korg*2;%FLOATING
KorgVAR(SALTCo>0.2)=P.Korg;
P.KorgVAR=KorgVAR;
...
%hold off
SP=A*0;SP(SALTCo<0.2)=0;SP(SALTCo>=0.2 & SALTCo<5)=1;SP(SALTCo>=5 & SALTCo<10)=2;SP(SALTCo>=10 & SALTCo<30)=3;
